...
#user_url = 'https://www.zhihu.com/api/v4/members/{user}?include={include}' #{ }括号内为变量，可以在后面传入
    answer_url = 'https://www.zhihu.com/api/v4/answers/{user}?include={include}&sort_by=created'  # { }括号内为变量，可以在后面传入
...
#user_query = 'locations,employments,gender,educations,business,voteup_count,thanked_Count,follower_count,following_count,cover_url,following_topic_count,following_question_count,following_favlists_count,following_columns_count,answer_count,articles_count,pins_count,question_count,commercial_question_count,favorite_count,favorited_count,logs_count,marked_answers_count,marked_answers_text,message_thread_token,account_status,is_active,is_force_renamed,is_bind_sina,sina_weibo_url,sina_weibo_name,show_sina_weibo,is_blocking,is_blocked,is_following,is_followed,mutual_followees_count,vote_to_count,vote_from_count,thank_to_count,thank_from_count,thanked_count,description,hosted_live_count,participated_live_count,allow_message,industry_category,org_name,org_homepage,badge[?(type=best_answerer)].topics'
    ...
    def start_requests(self):
        ...
        yield Request(self.answer_url.format(user=self.start_user, include=self.answer_query,limit=20, offset=0), self.parse_answer)

        # #请求vczh的查询页面，用parse_user方法解析
    ...
    def parse_comment(self, response):
        ...
        print(results)
        #print("value",result.get('question').get('id'))
    ...
    #     yield item
    #
    #     yield Request(                 # 获取轮子哥关注的用户，他们的关注者页面，用parse_follows方法解析
    #         self.follows_url.format(user=result.get('url_token'), include=self.follows_query, limit=20, offset=0),
    #         self.parse_follows)
    #
    #     yield Request(                 # 获取关注轮子哥的用户list页面，用parse_followers方法解析
